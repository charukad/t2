...
import { Avatar, Text, List, Divider, Button, Card, Snackbar, Badge, useTheme } from 'react-native-paper';
      ...
      const fetchData = async () => {
        try {
          ...
          console.log('DEBUG - User from Redux after loadUser:', 
            JSON.stringify({
              vehicleOwner: currentState.auth.user?.vehicleOwner || null,
          ...
          // Try to get cached vehicle owner data
          try {
            const cachedVehicleOwnerData = await AsyncStorage.getItem('cachedVehicleOwnerData');
            if (cachedVehicleOwnerData) {
              const parsedVehicleOwnerData = JSON.parse(cachedVehicleOwnerData);
              dispatch(updateProfile({ vehicleOwner: parsedVehicleOwnerData }));
            }
          } catch (cacheError) {
            console.log('Failed to load cached vehicle owner data:', cacheError);
          }
        }
      };
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.role}>Vehicle Owner</Text>
      </View>
      ...
      <View style={styles.section}>
        ...
        <List.Section>
          <List.Subheader>Vehicle Owner Information</List.Subheader>
          
          <List.Item
            title="License Number"
            description={user?.vehicleOwner?.licenseNumber || 'Not specified'}
          ...
          <List.Item
            title="Experience"
            description={user?.vehicleOwner?.experience ? `${user.vehicleOwner.experience} years` : 'Not specified'}
          ...
          <List.Item
            title="Service Areas"
            description={user?.vehicleOwner?.serviceAreas?.join(', ') || 'Not specified'}
            left={props => <List.Icon {...props} icon="map-marker-radius" />}
          />
        </List.Section>
        
        <List.Section>
          <List.Subheader>My Vehicles</List.Subheader>
          <Card style={styles.vehiclesCard}>
            <Card.Content>
              {user?.vehicleOwner?.vehicles && user.vehicleOwner.vehicles.length > 0 ? (
                user.vehicleOwner.vehicles.map((vehicle, index) => (
                  <View key={index} style={styles.vehicleItem}>
                    ...
                    <Text style={styles.vehicleText}>
                      Type: {vehicle.type || 'N/A'} | Capacity: {vehicle.capacity || 'N/A'} persons
                    </Text>
                    {index < user.vehicleOwner.vehicles.length - 1 && <Divider style={styles.vehicleDivider} />}
                  </View>
                ))
              ) : (
                <Text style={styles.bioText}>
                  No vehicles added yet. Add your vehicles to start offering services.
                </Text>
              )}
            </Card.Content>
            <Card.Actions>
